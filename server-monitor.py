    ...
    try:
        ...
        for server in js['servers']:
            ...
            if (badct.count(serverName) >= badct_notify_threshold) and (serverName not in badctnotify):
                if lang_uage == 'EN':
                    text = f"#ServerStatus {server_id}\n#{serverName} successful ping to CT *is less than* threshold\nCT: {server['ping_189']}"
                    _tgapi_call(text)
                else:
                    text = f"#ServerStatus {server_id}\n#{serverName} PING CT 成功率低于阈值\nCT: {server['ping_189']}"
                    _tgapi_call(text)
            ...
            ## else if server tcp connection is not too many and previously notified because of too many tcp, notify and remove it from too many tcp notify list
                ...
                if lang_uage == 'EN':
                    text = f"#ServerStatus {server_id}\n#{serverName} successful ping to CT return to normal\nCT: {server['ping_189']}"
                    _tgapi_call(text)
                else:
                    text = f"#ServerStatus {server_id}\n#{serverName} PING CT 成功率恢复正常\nCT: {server['ping_189']}"
                    _tgapi_call(text)
            ...
            if (badcm.count(serverName) >= badcm_notify_threshold) and (serverName not in badcmnotify):
                if lang_uage == 'EN':
                    text = f"#ServerStatus {server_id}\n#{serverName} successful ping to CM *is less than* threshold\nCM: {server['ping_10086']}"
                    _tgapi_call(text)
                else:
                    text = f"#ServerStatus {server_id}\n#{serverName} PING CM 成功率低于阈值\nCM: {server['ping_10086']}"
                    _tgapi_call(text)
            ...
            ## else if server tcp connecmion is not too many and previously notified because of too many tcp, notify and remove it from too many tcp notify list
                ...
                if lang_uage == 'EN':
                    text = f"#ServerStatus {server_id}\n#{serverName} successful ping to CM return to normal\nCM: {server['ping_10086']}"
                    _tgapi_call(text)
                else:
                    text = f"#ServerStatus {server_id}\n#{serverName} PING CM 成功率恢复正常\nCM: {server['ping_10086']}"
                    _tgapi_call(text)
